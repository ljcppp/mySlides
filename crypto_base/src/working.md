...
特别地，当a、b互素时，则一定存在整数x、y使得a\*x+b\*y=1成立。
...
\begin{aligned}
  ...
  \therefore m = k_1p\ \mathbf{or}\ m = k_2q \newline
\end{aligned}
$$

不妨设 $m = k_1p$，由欧拉定理有：$m^{\phi(q)} = 1\ mod\ q = tq + 1$
...
- 对于不同的大素数p构成的循环群$Z^*_p$，离散对数难度是相同的吗？
...
4. 再令$r=2,3,4,\cdots,k_i$，重复步骤3，依次求出 $a_1,a_2,\cdots,a_{k_{i−1}}$，整个的时间复杂度是 $O(p_ik_i)$。可以得到 $x = a_0 + a_1p_i + a_2p_i^2 + \cdots + a_{k_{i-1}}p_i^{k_{i-1}} \pmod{p_i^{k_i}}$
...
### 加法
$0x57 + 0x83 = (x^6+x^4+x^2+x+1) + (x^7+x+1) = x^7+x^6+x^4+x^2+2x+2 = x^7+x^6+x^4+x^2 = 0xd4$
